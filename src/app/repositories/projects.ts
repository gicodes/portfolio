export const projects = [
  {
    id: 'tictask',
    title: 'TicTask',
    name: 'TicTask',
    owner: 'gicodes',
    description: 'Future of workflow and task management. Smart ticketing app with AI assistance and useful resources. \n Built for fast, friendly, agile teamwork. Whether you are a small team or a large enterprise, TicTask is designed to meet your needs with its intuitive interface and robust features. \n',
    source: 'https://github.com/gicodes/TicTask',
    link: 'https://tictask.org',
    category: ['Productivity', 'AI',],
    license: 'MIT, Apache-2.0, GPL-3.0, BSD-3-Clause, MPL-2.0, CDDL-1.0, EPL-2.0, AGPL-3.0, Unlicense, CC0-1.0, OFL-1.1, Zlib, ISC, EUPL-1.1, LGPL-3.0',
    tech: ['NextJs', 'Typescript', 'ExpressJs', 'NodeJs', 'Postgres', 'OpenAI + Gemini API', 'MUI']
  },
  {
    id: 'hospital-plug',
    title: 'Hospital Plug',
    name: 'hospitalplug',
    owner: 'gicodes',
    description: 'A modern healthcare gateway designed to close the gap between people and hospitals. Whether you are at home, at work, or on the move, Hospital Plug helps you connect with the care you need quickly, reliably, and securely. \n Built with NextJs, Custom CSS, Google Maps, ExpressJs + NodeJs \n',
    source: 'https://github.com/gicodes/hositalplug',
    link: 'https://hospitalplug.com',
    license: 'MIT, Apache-2.0, GPL-3.0, BSD-3-Clause, MPL-2.0, CDDL-1.0, EPL-2.0, AGPL-3.0, Unlicense, CC0-1.0, OFL-1.1, Zlib, ISC, EUPL-1.1, LGPL-3.0',
    category: ['Healthcare', 'E-commerce'],
    tech: ['NextJs', 'Custom CSS', 'Google Maps', 'ExpressJs', 'NodeJs']
  },
  {
    id: 'ukanwears',  
    title: 'Ukan29',
    name: 'ukanwears',
    owner: 'gicodes',
    description: 'Ukan29 is an international clothing brand that offer seamless e-commerce experience.\n Features some modern designs, with cool products and services. \n This app is built on Next + React, bootstrapped with typescript and postgres. Uses paypal and paystack for (worldwide) payment solution. \n',
    source: 'https://github.com/gicodes/ukanwears',
    link: 'https://ukanwears.com',
    license: 'MIT, Apache-2.0, GPL-3.0, BSD-3-Clause, MPL-2.0, CDDL-1.0, EPL-2.0, AGPL-3.0, Unlicense, CC0-1.0, OFL-1.1, Zlib, ISC, EUPL-1.1, LGPL-3.0',
    category: ['E-commerce', 'Fashion'],
    tech: ['NextJs', 'React', 'Typescript', 'Postgres', 'PayPal API', 'Paystack API']
  },
  {
    id: 'phoenix-medicals',
    title: 'Phoenix Medicals',
    name: 'pheonix-med',
    owner: 'gicodes',
    description: 'Bringing tech to health care with this modern, containerized web app.\n Phoenix Med runs a NextJs client that handles multiple user profiles with role-based functions, allows users book, schedule and instantly employ health care professionals.\n Bootstrapped with express, nodeJs and postgres. \n ',
    source: 'https://github.com/gicodes/pheonix-med',
    link: 'https://pheonixmed.netlify.app',
    license: 'MIT, Apache-2.0, GPL-3.0, BSD-3-Clause, MPL-2.0, CDDL-1.0, EPL-2.0, AGPL-3.0, Unlicense, CC0-1.0, OFL-1.1, Zlib, ISC, EUPL-1.1, LGPL-3.0',
    category: 'Healthcare',
    tech: ['NextJs', 'React', 'Typescript', 'Custom SCSS', 'Postgres']
  },
  {
    id: 'afro-fashion',
    title: 'Afro Fashion',
    name: 'afro-fashion',
    owner: 'gicodes',
    description: 'E-commerce platform that connects sellers to buyers and handles payments.\n Afrofashion is built on FERN stack with React, Express and NodeJs tightly coupled to deliver a beautiful user-experience \n This app is bootstrapped with custom SCSS, flutterwave and firebase, google auth for user and role-based functions. \n',
    source: 'https://github.com/gicodes/afro-fashion',
    link: 'https://afrofashionng.netlify.app', 
    license: 'MIT, Apache-2.0, GPL-3.0, BSD-3-Clause, MPL-2.0, CDDL-1.0, EPL-2.0, AGPL-3.0, Unlicense, CC0-1.0, OFL-1.1, Zlib, ISC, EUPL-1.1, LGPL-3.0',
    category: ['E-commerce', 'Fashion'],
    tech: ['React', 'Custom SCSS', 'ExpressJs', 'NodeJs', 'Firebase', 'Flutterwave API', 'Google Auth API']
  },
  {
    id: 'attribution-js',
    title: 'Attribution Js',
    name: 'attribution.js',
    owner: 'gicodes',
    description: 'Express-based API designed to track user interaction on e-commerce products. \n Attribution Reporting provisions a reliable backend service that Developers and Analytical Engineers can use to optimize performance, sales campaign effectiveness and manage Ads with metrics and metadata on Client applications \n ',
    source: 'https://github.com/gicodes/attribution.js',
    link: 'https://tourcan.org', // showing tourcan website here signifies attirbution can be connected to any client application
    category: 'Productivity',
    tech: ['ExpressJs', 'NodeJs', 'Postgres']
  },
  {
    id: 'mechtatel',
    title: 'Mechtatel',
    name: 'mechtatel',
    owner: 'gicodes',
    description: 'NextJs SPA as landing page for HR consultancy services. \n Mechtatel is built with React + Typescript on Next 15 and is bootstrapped with React-bootstrap and Tailwind CSS to serve an eye-catching UI for users, hiring managers and other business enterprise. \n',
    source: 'https://github.com/gicodes/mechtatel',
    link: 'https://mechtatel.vercel.app',
    category: 'Productivity',
    tech: ['NextJs', 'React', 'Typescript', 'MUI']
  },
]